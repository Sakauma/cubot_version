...
void get_surround_pts( std::vector<struct s_bar> &bar_list, const cv::Mat img_backups )
{
    for ( int i = 0; i < bar_list.size(); i++ )
    {
        ...
        get_expand_pts( bar_list[i].endpoints, &bar_list[i].ex_small_armor, angle, 0, 2 * h, 2.5 * h, img_backups.rows, img_backups.cols );
        get_proposal_pts( bar_list[i].endpoints, angle, 1 * h, 2.5 * h, &bar_list[i].prop_small_armor, img_backups.rows, img_backups.cols );
        get_expand_pts( bar_list[i].endpoints, &bar_list[i].ex_large_armor, angle, 0, 4 * h, 4.0 * h, img_backups.rows, img_backups.cols );
    }

}

static void prepare_transform_img( struct s_bar *pDomain_bar, const cv::Mat & img_backups, int small_or_big )
{
...
int find_small_armor_in_expts(struct s_bar *pDomain_bar, const struct s_bar *pSlave_bar, float distance_limit, float angle_limit, const cv::Mat &img_backups )
{
    int result = 0;
    int match_result = 0;
    ...
    prepare_transform_img( pDomain_bar, img_backups, 0 );
    result = AutoHit::predict( AutoHit::model, AutoHit::pHog, AutoHit::img_transform, AutoHit::Model_cnt );
    return result;

}

int find_small_armor_in_props( struct s_bar *pDomain_bar, float area_k, cv::Mat & img_backups )
{
    int result = 0;
...
int find_large_armor_in_expts( struct s_bar *pDomain_bar, const struct s_bar *pSlave_bar, float distance_limit, float angle_limit, const cv::Mat &img_backups )
{
    int result = 0;
    int match_result = 0;
    ...
    prepare_transform_img( pDomain_bar, img_backups, 1 );
    result = AutoHit::predict( AutoHit::model, AutoHit::pHog, AutoHit::img_transform, AutoHit::Model_cnt );
    return result;
}
...
int find_armor_int_all_pts( cv::Mat &img_src, std::vector<struct s_bar>  &valid_bar_list, cv::Mat &img_backups, bool &is_small_armor )
{
    int result = 0;
    ...
    for ( int i = 0; i < valid_bar_list.size(); i++ )
    {
        for ( int j = 0; j < valid_bar_list.size(); j++ )
        {
            ...
            is_small_armor = true;
            result = find_small_armor_in_expts( &valid_bar_list[i], &valid_bar_list[j], 0.3, AutoHit::small_pair_bar_angle_limit, img_backups );
            if ( result != 0 )
                 return i;

            result = find_small_armor_in_props( &valid_bar_list[i], AutoHit::proposal_bar_area_k, img_backups );
            if ( result != 0 )
                 return i;

            is_small_armor = false;
            result = find_large_armor_in_expts( &valid_bar_list[i], &valid_bar_list[j], 1.0, AutoHit::big_pair_bar_angle_limit, img_backups );
            if ( result != 0 )
                 return i;
        }
    };
