...
int check_rect( cv::Rect &r, int rows, int cols )
{
    if ( (r.x * r.y < 0 ) ||\
    ( r.x + r.width >= cols ) ||\
    ( r.x <= 0 ) ||\
...
void get_armor_from_pts( cv::Point2f e1, cv::Point2f e2, cv::Point2f p1, cv::Point2f p2, cv::Point2f *pArmor, int rows, int cols )
{
    ...
    pArmor[0].x = temp[0].x + 0.5 * dx;
    pArmor[0].y = temp[0].y + 0.5 * dy;
    pArmor[3].x = temp[3].x - 0.5 * dx;
    pArmor[3].y = temp[3].y - 0.5 * dy;

    dx = temp[1].x - temp[2].x;
    dy = temp[1].y - temp[2].y;
    pArmor[1].x = temp[1].x + 0.5 * dx;
    pArmor[1].y = temp[1].y + 0.5 * dy;
    pArmor[2].x = temp[2].x - 0.5 * dx;
    pArmor[2].y = temp[2].y - 0.5 * dy;


    pArmor[0].x = limit_number( pArmor[0].x, cols, 0 );
    pArmor[1].x = limit_number( pArmor[1].x, cols, 0 );
    pArmor[2].x = limit_number( pArmor[2].x, cols, 0 );
    pArmor[3].x = limit_number( pArmor[3].x, cols, 0 );

    pArmor[0].y = limit_number( pArmor[0].y, rows, 0 );
    pArmor[1].y = limit_number( pArmor[1].y, rows, 0 );
    pArmor[2].y = limit_number( pArmor[2].y, rows, 0 );
    pArmor[3].y = limit_number( pArmor[3].y, rows, 0 );
}
...
void sort_ellipse( const std::vector<struct s_bar> &bars_src, std::vector<struct s_bar> &bars_dst, int x, int y )
{
    ...
    else
    {
        ...
        {
             distance.push_back( pow( ( bars_src[i].ellipse.center.x - x ), 2 ) + pow( ( bars_src[i].ellipse.center.y - y ), 2 ) );
        }

        new_indexs = sort_indexes( distance );
...
void sort_bar_according_to_height( const std::vector<struct s_bar> &bars_src, std::vector<struct s_bar> &bars_dst )
{
    ...
    else
    {
        ...
        for ( int i = 0; i < ori_bars.size(); i++ )
              heghts.push_back( bars_src[i].ellipse.size.height );
