...
namespace AutoHit
{
    ...
    extern cv::Ptr<cv::ml::SVM> model ;
    extern cv::HOGDescriptor *pHog ;

    extern int Model_cnt;
    extern int autohit_flag;

    int predict( cv::Ptr<cv::ml::SVM> &model, cv::HOGDescriptor *pHog, cv::Mat &img_transform, int &model_cnt );

    void transform_armor_vertex( cv::Point2f *pPts, float k );
    cv::Point2f detection( cv::Mat &img, float &dist, int &is_small_armor );
}
