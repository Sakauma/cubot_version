...
static void detect_init( void )
{
    ...
    if ( file_2.isOpened() )
    {
        ...
        std::string svm_model_path  = ( std::string )file_2["svm_model_path"];   //这个未改

        autohit::color_id = ( int )fs2["color2Strike"];
        ...
        autohit::model = cv::ml::SVM::load( "../cfg/hero_svm_model.xml" );
...
void get_hight_light_bar( const cv::Mat & img_src, cv::Mat & mask, int color_id )
{
    ...
    if ( color_id == 0 )
    {
        weighted_cvt_gray( img_src, weighted_gray, color_id );
...
int meng::predict( cv::Ptr<cv::ml::SVM> &model, cv::HOGDescriptor *pHog, cv::Mat &img_transform, int &model_cnt )
{
    ...
    cv::Mat high_light_roi_img = img_transform( high_light_roi_rect );

    int pxl_cnt = 0;
    if ( color_id == 0 )
    {
        for ( int i = 0; i < high_light_roi_img.rows; i++ )
        {
            uchar *data = high_light_roi_img.ptr<uchar>(i);
            for ( int j = 0; j < high_light_roi_img.cols; j++ )
            {
                  uchar b = data[ j * 3 ];
                  uchar g = data[ j * 3 + 1 ];
                  uchar r = data[ j * 3 + 2 ];
...
void meng::transform_armor_vertex( cv::Point2f *pPts, float k )
{
...
//                        printf("bar %d is not valid +++++++++\n",contour_i);
    ...
    if( sp.x != -1 ) {
        ...
        if (key == 13){//Enter
            ...
            cv::imwrite( "/home/snow/imgs/"+std::to_string(715+roi_cnt++)+".jpg",e);
            printf("save roi\n");
        }
    }
    //cv::waitKey();
