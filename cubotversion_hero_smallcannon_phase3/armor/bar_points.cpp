                    ...
                    float distance1, float distance2, int rows, int cols )
{
    ...
    find_newpoint( pEndpoints[0], sup_angle, -distance2, ( *pEx_pts ).horizon_l[0], rows, cols );
    find_newpoint( pEndpoints[1], sup_angle, -distance2, ( *pEx_pts ).horizon_l[1], rows, cols );
    check_endpoint( ( *pEx_pts ).horizon_l[0], ( *pEx_pts ).horizon_l[1] );

    find_newpoint( pEndpoints[0], sup_angle, distance2, ( *pEx_pts ).horizon_r[0], rows, cols );
    find_newpoint( pEndpoints[1], sup_angle, distance2, ( *pEx_pts ).horizon_r[1], rows, cols );
                             ...
                             float distance_limit, float angle_limit, int small_or_large )
{
    ...
    if ( small_or_large == 0 )
    {
         ...
         angle_diff = abs( ( *pDomain_bar).ellipse.angle - ( *pSlave_bar ).ellipse.angle );
...
std::vector<int> match_two_bars_by_propts( struct proposal_pt prop_pts, float area_k, cv::Mat &img_backups )
{
    ...
    if ( check_rect( proposal, img_backups.rows, img_backups.cols ) )
    {
         float count_area = cv::contourArea( proposal_vec );

         if ( check_mask_area( img_backups, proposal ) / count_area > area_k )
              result[0]=1;
    }
...
void get_surround_pts( std::vector<struct s_bar> &bar_list, const cv::Mat &img_backups )
{
    for ( int i = 0; i < bar_list.size(); i++ )
    {
          ...
          get_expand_pts( bar_list[i].pEndpoints, &bar_list[i].ex_small_armor, angle, 0, 2 * h, 2.5 * h, img_backups.rows, img_backups.cols );
          get_proposal_pts( bar_list[i].pEndpoints, angle, 1 * h, 2.5 * h, &bar_list[i].prop_small_armor, img_backups.rows, img_backups.cols );
          get_expand_pts( bar_list[i].pEndpoints, &bar_list[i].ex_large_armor, angle, 0, 4 * h, 4.0 * h, img_backups.rows, img_backups.cols );
    }
...
static void prepare_transform_img(cv::Mat &img_src, struct s_bar *pDomain_bar, const cv::Mat & img_backups, int small_or_big, bool isDrawLine )
{
    ...
    if ( isDrawLine )
    {
        ...
        cv::line( img_src, autohit::new_pts[1], autohit::new_pts[2], cv::Scalar(0, 0, 255), 1 );
        cv::line( img_src, autohit::new_pts[2], autohit::new_pts[3], cv::Scalar(0, 255, 255), 1 );
        cv::line( img_src, autohit::new_pts[3], autohit::new_pts[0], cv::Scalar(0, 255, 0), 1 );
    }
...
int find_small_armor_in_props( cv::Mat &img_src, struct s_bar *pDomain_bar, float area_k, cv::Mat & img_backups )
{
    ...
    {
        find_newpoint( ( *pDomain_bar ).endpoints[0],angle,2*h,(*domain_bar).pair_p1,img_ori.rows,img_ori.cols);
        find_newpoint( ( *pDomain_bar ).endpoints[1],angle,2*h,(*domain_bar).pair_p2,img_ori.rows,img_ori.cols);
        check_endpoint( ( *pDomain_bar ).pair_p1,(*domain_bar).pair_p2);
        ...
        result = meng::predict(meng::model,meng::hog,meng::transform_img,meng::Model_cnt); //model->predict(t_equ);
...
int find_large_armor_in_expts(cv::Mat &img_src, struct s_bar *pDomain_bar, const struct s_bar *pSlave_bar, float distance_limit, float angle_limit, cv::Mat & img_backups )
{
    ...
    float h_div = pDomain_bar->ellipse.size.height / slave_bar->ellipse.size.height;
    if ( h_div < 0.55 )
    {
        ( *pDomain_bar ).pair_p1 = ( *pSlave_bar ).endpoints[0];
        ( *pDomain_bar ).pair_p2 = ( *pSlave_bar ).endpoints[1];
...
int find_armor_int_all_pts( cv::Mat &img_src, std::vector<struct s_bar> &valid_bar_list, cv::Mat &img_backups )
{
    ...
    for ( int i = 0; i < valid_bar_list.size(); i++ )
    {
        for ( int j = 0; j < valid_bar_list.size(); j++ )
        {
            ...
            result = find_large_armor_in_expts( img_src, &valid_bar_list[i], &valid_bar_list[j],5., autohit::big_pair_bar_angle_limit, img_backups );
            if ( result != 0 )
                 return i;
        }
    };
