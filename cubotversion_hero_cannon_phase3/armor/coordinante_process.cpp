...
{
    cv::Mat mask_bgr, color_bin_mask;
    mask_bgr = img_bgr( roi );

    float area = 0;
    if ( mask_bgr.channels() == 1 )
    {
        area = cv::sum( mask_bgr )[0] / 255.;
    }
    else if ( mask_bgr.channels() == 3 )
    {
        if ( AutoHit::color_id == 0 )
        {
            cv::inRange( mask_bgr, cv::Scalar(100, 100, 100), cv::Scalar(124, 255, 255), color_bin_mask );
        }
        else if ( AutoHit::color_id == 1 )
        {
            cv::Mat hsv1, hsv2;
            cv::inRange( mask_bgr, cv::Scalar(0, 70, 70), cv::Scalar(10, 200, 255), hsv1 );
            cv::inRange( mask_bgr, cv::Scalar(170, 70, 70), cv::Scalar(180, 200, 255), hsv2 );
...
void get_armor_from_pts( cv::Point2f e1, cv::Point2f e2, cv::Point2f p1, cv::Point2f p2, cv::Point2f *pArmor, int rows, int cols )
{
    ...
    float stripe_length=sqrt ( pow( temp[0].x - temp[3].x, 2 ) + pow( temp[0].y - temp[3].y, 2 ) );
    float armor_length=sqrt( pow( temp[0].x - temp[1].x, 2 ) + pow( temp[0].y - temp[1].y, 2  ) );
    if ( 6.5 * stripe_length < armor_length )
    {
        k1 = 0.2;
        k2 = 0.8;
    }
    ...
    pArmor[0].x = temp[0].x + k1 * dx;
    pArmor[0].y = temp[0].y + k2 * dy;
    pArmor[3].x = temp[3].x - k1 * dx;
    pArmor[3].y = temp[3].y - k2 * dy;

    dx = temp[1].x - temp[2].x;
    dy = temp[1].y - temp[2].y;
    pArmor[1].x =  temp[1].x + k1*dx;
    pArmor[1].y =  temp[1].y + k2*dy;
    pArmor[2].x =  temp[2].x - k1*dx;
    pArmor[2].y =  temp[2].y - k2*dy;
...
void sort_ellipse( const std::vector<struct s_bar> &bars_ori, std::vector<struct s_bar> &bars_dst, int x, int y )
{
    ...
    else
    {
        ...
        for (int i=0; i<bars_ori.size(); i++ )
        {
            distance.push_back( pow( ( bars_ori[i].ellipse.center.x - x ), 2 ) + pow( ( bars_ori[i].ellipse.center.y - y ), 2 ) );
        }
        ...
        {
            bars_dst.push_back( bars_ori[new_indexs[i]] );
        }
        return ;
...
void sort_bar_according_to_height( const std::vector<struct s_bar> &bars_ori, std::vector<struct s_bar> &bars_dst )
{
    ...
    {
        std::vector<struct s_bar> bars_temp =bars_ori;
    ...
    else
    {
        ...
        for ( int i = 0; i < bars_ori.size(); i++ )
              heghts.push_back( bars_ori[i].ellipse.size.height );
