...
/*********************************
 *
 * Description: check if roi in mask contain some points
 * Input :
 *      1. img_bgr
 * Output:
 *      1. color_bin_mask
 ...
 * Output：
 *      1.  cv::Point2f *pArmor ((*domain_bar).armor_vertex)
 * Others：
 ...
 * Input:
 *      1.  bars_ori（tmp_valid_bar_list）  灯条信息
 *      2.  x（autohit::dynamic_sp.x）  虚拟的目标点x坐标
 *      3.  y（autohit::dynamic_sp.y）  虚拟的目标点y坐标
 * Output：
 *      1.  bars_dst（valid_bar_list）  升序排序后的灯条信息
 * Others：
...
void sort_bar_according_to_height( const std::vector<struct s_bar> &bars_ori, std::vector<struct s_bar> &bars_dst )
{
    ...
    }else
    {
        ...
        //遍历每个灯条
        for ( int i=0; i<bars_ori.size(); i++ )
            heghts.push_back( bars_ori[i].ellipse.size.height );      //将所以灯条的外接椭圆长轴存储在 heghts 中
